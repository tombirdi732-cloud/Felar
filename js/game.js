  ...
  for (let r = 0; r < lvl.rows; r++) {
    for (let c = c0; c <= c1; c++) {
      ...
      if (t === "#") {
        ...
        // декор поверх земли (утоплен в мох, чтобы скрыть основание)
        if (topOpen && lvl.tile(c, r - 1) === " ") {
          const h = hash2(c, r * 31 + G.chapter);
          if (h > 0.93) ctx.drawImage(ts, 4 * T2, 0, T2, T2, x, y - TILE + 10, TILE, TILE);
          else if (h < 0.05) ctx.drawImage(ts, 5 * T2, 0, T2, T2, x, y - TILE + 10, TILE, TILE);
        }
      } else if (t === "=") {
        ctx.drawImage(ts, 2 * T2, 0, T2, T2, x, y, TILE, TILE);
      } else if (t === "^") {
        // шипы утоплены: их основание перекроется блоком земли ниже
        ctx.drawImage(ts, 3 * T2, 0, T2, T2, x, y + 8, TILE, TILE);
      } else if (t === "v") {
        ctx.save();
        ctx.translate(x + TILE / 2, y + TILE / 2 - 8);
        ctx.scale(1, -1);

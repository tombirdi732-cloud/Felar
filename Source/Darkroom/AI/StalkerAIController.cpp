...
void AStalkerAIController::OnPossess(APawn* InPawn)
{
	...
	if (!StalkerPawn)
	{
		UE_LOG(LogDarkroom, Warning, TEXT("StalkerAIController possessed non-Stalker pawn %s"), *GetNameSafe(InPawn));
		return;
	}
...
void AStalkerAIController::SetState(EStalkerState NewState)
{
	...
	UE_LOG(LogDarkroom, Verbose, TEXT("Stalker state: %d -> %d"), (int32)OldState, (int32)NewState);
...
{
	ADarkroomCharacter* Player = Cast<ADarkroomCharacter>(Actor);
...
void AStalkerAIController::RefreshSightRadius()
{
	...
	const ADarkroomCharacter* Player = Cast<ADarkroomCharacter>(
	...
	UE_LOG(LogDarkroom, Verbose, TEXT("Stalker sight radius -> %.0f (flashlight %s)"),
...
void AStalkerAIController::StartChase(ADarkroomCharacter* Target)
{
	ChaseTarget = Target;

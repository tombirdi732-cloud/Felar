 ...
 */
UCLASS()
class DARKROOM_API AStalkerAIController : public AAIController
{
	GENERATED_BODY()
...
protected:
	...
	void StartChase(ADarkroomCharacter* Target);
	void StartSearch();
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Darkroom|AI")
	FName PatrolPointTag = TEXT("PatrolPoint");

	/** Радиус случайного блуждания, если точек патрулирования на карте нет. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Darkroom|AI", meta = (ClampMin = "100.0"))
	float RandomPatrolRadius = 4000.f;

	/** Пауза на месте после прихода в точку патрулирования. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Darkroom|AI", meta = (ClampMin = "0.0"))
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Darkroom|AI", meta = (ClampMin = "0.05"))
	float ChaseRepathInterval = 0.35f;

	/** Как часто пересчитывать радиус зрения под состояние фонаря. Секунды. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Darkroom|AI", meta = (ClampMin = "0.1"))
	float SightRefreshInterval = 0.5f;

	/** Насколько близко нужно подойти к точке, чтобы считать её достигнутой. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Darkroom|AI", meta = (ClampMin = "10.0"))
	float AcceptanceRadius = 90.f;

private:
	UPROPERTY(VisibleDefaultsOnly, Category = "Darkroom|AI")
	...
	UPROPERTY(Transient)
	TObjectPtr<ADarkroomCharacter> ChaseTarget;

...
FText APickupActor::GetInteractionPrompt_Implementation(ADarkroomCharacter* Interactor) const
{
	return PickupPrompt;
}
...
void APickupActor::Interact_Implementation(ADarkroomCharacter* Interactor)
{
	if (!Interactor)
	{
		return;
...
void APickupActor::OnCollected(ADarkroomCharacter* Collector)
{
...
{
	PickupPrompt = NSLOCTEXT("Darkroom", "PickupFragment", "Забрать фрагмент");
	FearRelief = 12.f;
...
void AFragmentPickup::OnCollected(ADarkroomCharacter* Collector)
{
	...
	{
		UE_LOG(LogDarkroom, Warning, TEXT("Fragment collected but GameMode is not ADarkroomGameMode"));
	}
}
...
{
	PickupPrompt = NSLOCTEXT("Darkroom", "PickupBattery", "Взять батарею");
	FearRelief = 5.f;
...
void ABatteryPickup::OnCollected(ADarkroomCharacter* Collector)
{

 ...
 */
UCLASS()
class DARKROOM_API ANightAtmosphere : public AActor
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Darkroom|Night")
	void ApplyAtmosphere();

	/** Плавно изменить плотность тумана — для скриптовых сцен. */
	UFUNCTION(BlueprintCallable, Category = "Darkroom|Night")
	void SetFogDensityImmediate(float NewDensity);

protected:
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Darkroom|Night")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Moon", meta = (ClampMin = "-90.0", ClampMax = "90.0"))
	float MoonPitch = -32.f;

	/** Направление луны по горизонту в градусах. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Moon", meta = (ClampMin = "-360.0", ClampMax = "360.0"))
	float MoonYaw = 145.f;
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Moon", meta = (ClampMin = "0.0", ClampMax = "5.0"))
	float MoonIntensity = 0.35f;

	/** Холодный синий — то, как мозг ожидает увидеть лунный свет. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Moon")
	FLinearColor MoonColor = FLinearColor(0.42f, 0.55f, 0.95f, 1.f);

	/** Угловой размер источника: больше — мягче тени. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Moon", meta = (ClampMin = "0.0", ClampMax = "20.0"))
	float MoonSourceAngle = 1.2f;

	/** Насколько луна светится сквозь туман. Это и есть лучи в окнах. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Moon", meta = (ClampMin = "0.0", ClampMax = "10.0"))
	float MoonVolumetricScattering = 3.5f;

	/** Дальность динамических теней в сантиметрах. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Moon", meta = (ClampMin = "500.0"))
	float ShadowDistance = 12000.f;

	// --- Небо и ambient ---

	/** Общая подсветка теней. Ноль — абсолютно чёрные тени, играть невозможно. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Sky", meta = (ClampMin = "0.0", ClampMax = "2.0"))
	float SkyLightIntensity = 0.12f;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Sky")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Fog", meta = (ClampMin = "0.0", ClampMax = "0.5"))
	float FogDensity = 0.035f;

	/** Скорость спадания тумана с высотой. Больше — туман жмётся к земле. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Fog", meta = (ClampMin = "0.001", ClampMax = "2.0"))
	float FogHeightFalloff = 0.15f;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Fog")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Fog", meta = (ClampMin = "0.0"))
	float FogStartDistance = 150.f;

	/** Максимальная непрозрачность. Ниже единицы — вдали не белая стена, а дымка. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Fog", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Fog", meta = (ClampMin = "0.0", ClampMax = "10.0"))
	float VolumetricExtinctionScale = 1.6f;

	/** Дальность просчёта объёмного тумана в сантиметрах. Дороже всего именно она. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Darkroom|Night|Fog", meta = (ClampMin = "1000.0"))

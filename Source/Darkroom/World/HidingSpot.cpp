...
bool AHidingSpot::CanInteract_Implementation(ADarkroomCharacter* Interactor) const
{
	return !IsOccupied() && Interactor && Interactor->IsAlive();
}

FText AHidingSpot::GetInteractionPrompt_Implementation(ADarkroomCharacter* Interactor) const
{
	return IsOccupied()
		? NSLOCTEXT("Darkroom", "HidingSpotOccupied", "Занято")
		: NSLOCTEXT("Darkroom", "HidingSpotEnter", "Спрятаться");
}

void AHidingSpot::Interact_Implementation(ADarkroomCharacter* Interactor)
{
...
void AHidingSpot::NotifyPlayerLeft(ADarkroomCharacter* Player)
{
	if (Occupant != Player)
	{
		return;

...
ADarkroomCharacter::ADarkroomCharacter()
{
...
void ADarkroomCharacter::BuildInputMappings()
{
	...
	InputMapping = CreateDefaultSubobject<UInputMappingContext>(TEXT("IMC_Darkroom"));
...
void ADarkroomCharacter::BeginPlay()
{
	Super::BeginPlay();
	...
	FearComponent->OnInvoluntaryGasp.AddDynamic(this, &ADarkroomCharacter::HandleInvoluntaryGasp);
	FearComponent->OnFearBreakdown.AddDynamic(this, &ADarkroomCharacter::HandleFearBreakdown);

	LastStepLocation = GetActorLocation();
	UpdateMaxSpeed();
}

void ADarkroomCharacter::Tick(float DeltaTime)
{
...
void ADarkroomCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	Input->BindAction(IA_MoveForward, ETriggerEvent::Triggered, this, &ADarkroomCharacter::HandleMoveForward);
	Input->BindAction(IA_MoveRight, ETriggerEvent::Triggered, this, &ADarkroomCharacter::HandleMoveRight);
	Input->BindAction(IA_Look, ETriggerEvent::Triggered, this, &ADarkroomCharacter::HandleLook);
	Input->BindAction(IA_LookGamepad, ETriggerEvent::Triggered, this, &ADarkroomCharacter::HandleLookGamepad);

	Input->BindAction(IA_Sprint, ETriggerEvent::Started, this, &ADarkroomCharacter::HandleSprintStarted);
	Input->BindAction(IA_Sprint, ETriggerEvent::Completed, this, &ADarkroomCharacter::HandleSprintCompleted);
	Input->BindAction(IA_Crouch, ETriggerEvent::Started, this, &ADarkroomCharacter::HandleCrouchPressed);
	Input->BindAction(IA_Interact, ETriggerEvent::Started, this, &ADarkroomCharacter::HandleInteractPressed);
	Input->BindAction(IA_Flashlight, ETriggerEvent::Started, this, &ADarkroomCharacter::HandleFlashlightPressed);
}

void ADarkroomCharacter::RegisterInputMapping()
{
	...
	if (!Subsystem)
	{
		UE_LOG(LogDarkroom, Error, TEXT("EnhancedInput subsystem not available"));
		return;
	}
...
void ADarkroomCharacter::HandleMoveForward(const FInputActionValue& Value)
{
...
void ADarkroomCharacter::HandleMoveRight(const FInputActionValue& Value)
{
	const float Scale = Value.Get<float>();
	if (Scale == 0.f || !bAlive || IsHiding())
	{
...
void ADarkroomCharacter::HandleLook(const FInputActionValue& Value)
{
...
void ADarkroomCharacter::HandleLookGamepad(const FInputActionValue& Value)
{
	const FVector2D Look = Value.Get<FVector2D>();
...
void ADarkroomCharacter::HandleSprintStarted()
{
	bWantsToSprint = true;
	UpdateMaxSpeed();
}

void ADarkroomCharacter::HandleSprintCompleted()
{
	bWantsToSprint = false;
	UpdateMaxSpeed();
}

void ADarkroomCharacter::HandleCrouchPressed()
{
	if (IsHiding())
	{
		return;
...
void ADarkroomCharacter::HandleInteractPressed()
{
	if (!bAlive)
	{
		return;
...
void ADarkroomCharacter::HandleFlashlightPressed()
{
	if (!bAlive)
	{
		return;
...
void ADarkroomCharacter::TickStamina(float DeltaTime)
{
...
void ADarkroomCharacter::TickFootsteps(float DeltaTime)
{
...
void ADarkroomCharacter::Landed(const FHitResult& Hit)
{
	Super::Landed(Hit);
...
void ADarkroomCharacter::TickStalkerVisibility(float DeltaTime)
{
	...
	// ...и не за стеной.
	FCollisionQueryParams Params(SCENE_QUERY_STAT(DarkroomStalkerVisibility), false, this);
...
void ADarkroomCharacter::UpdateMaxSpeed()
{
	UCharacterMovementComponent* Movement = GetCharacterMovement();
...
ENoiseLevel ADarkroomCharacter::GetCurrentMovementNoise() const
{
	if (bIsCrouched)
	{
...
void ADarkroomCharacter::EmitNoise(ENoiseLevel Level, FVector OverrideLocation)
{
	if (!bAlive)
	{
		return;
	...
	const float Loudness = DarkroomNoise::ToLoudness(Level);
...
void ADarkroomCharacter::HandleInvoluntaryGasp()
{
	// Спрятавшийся игрок всё равно ахает — укрытие не делает его бесшумным.
	EmitNoise(ENoiseLevel::Quiet);
	UE_LOG(LogDarkroom, Verbose, TEXT("Player gasped from fear"));
}

void ADarkroomCharacter::HandleFearBreakdown()
{
	...
	UE_LOG(LogDarkroom, Log, TEXT("Player fear breakdown - position revealed"));
}

// --- Состояния ---

bool ADarkroomCharacter::IsInLight() const
{
	return Flashlight->IsLightOn() || LitZoneCount > 0;
}

void ADarkroomCharacter::AddLitZone(int32 Delta)
{
	LitZoneCount = FMath::Max(0, LitZoneCount + Delta);
}

void ADarkroomCharacter::EnterHidingSpot(AHidingSpot* Spot)
{
...
void ADarkroomCharacter::LeaveHidingSpot()
{
	if (!IsHiding())
	{
		return;
...
void ADarkroomCharacter::OnCaught(AActor* Killer)
{
	if (!bAlive)
	{
		return;
	...
	UE_LOG(LogDarkroom, Log, TEXT("Player caught by %s"), *GetNameSafe(Killer));

	OnPlayerCaught.Broadcast(Killer);

	if (ADarkroomGameMode* GameMode = GetWorld()->GetAuthGameMode<ADarkroomGameMode>())
	{
		GameMode->FinishGame(EGameOutcome::Caught);
	}
}

AStalkerCharacter* ADarkroomCharacter::FindStalker()
{

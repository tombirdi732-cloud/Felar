...
void ASafeLightZone::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	Volume->GetOverlappingActors(Overlapping, AFelarCharacter::StaticClass());

	for (AActor* Actor : Overlapping)
	{
		if (AFelarCharacter* Player = Cast<AFelarCharacter>(Actor))
		{
			Player->AddLitZone(-1);
		}
	}
...
{
	if (AFelarCharacter* Player = Cast<AFelarCharacter>(OtherActor))
	{
		Player->AddLitZone(1);
	}
}

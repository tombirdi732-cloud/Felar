 ...
 */
UCLASS()
class FELAR_API AVHSHud : public AHUD
{
	GENERATED_BODY()
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Felar|VHS HUD")
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Felar|VHS HUD", meta = (ClampMin = "0.0", ClampMax = "0.3"))
	float MarginFraction = 0.06f;

	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Felar|VHS HUD", meta = (ClampMin = "0.5", ClampMax = "4.0"))
	float TextScale = 1.15f;

	/** Период мигания точки REC в секундах. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Felar|VHS HUD", meta = (ClampMin = "0.1"))
	 ...
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Felar|VHS HUD", meta = (ClampMin = "2", ClampMax = "16"))
	int32 ScanlineSpacing = 4;

	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Felar|VHS HUD", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	float ScanlineOpacity = 0.12f;

private:
	void DrawRecIndicator(float Margin);
	void DrawTimecode(float Margin);
	void DrawBattery(float Margin, const AFelarCharacter* Player);
	void DrawObjective(float Margin);
	void DrawInteractionPrompt(const AFelarCharacter* Player);
